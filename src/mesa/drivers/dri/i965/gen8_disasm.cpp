...
static int
three_source_type_to_reg_type(int three_source_type)
{
...
static int
dest(FILE *file, gen8_instruction *inst)
{
   int err = 0;

   if (inst->access_mode() == BRW_ALIGN_1)
   {
      assert(inst->dst_address_mode() == BRW_ADDRESS_DIRECT);
      err |= reg(file, inst->dst_reg_file(), inst->dst_da_reg_nr());
      if (err == -1)
         return 0;
      if (inst->dst_da1_subreg_nr())
         format(file, ".%d", inst->dst_da1_subreg_nr() /
                reg_type_size[inst->dst_reg_type()]);
      string(file, "<");
      err |= control(file, "horiz stride", m_horiz_stride, inst->dst_da1_hstride(), NULL);
      string(file, ">");
      err |= control(file, "dest reg encoding", m_reg_type, inst->dst_reg_type(), NULL);
   }
   else
   {
      assert(inst->dst_address_mode() == BRW_ADDRESS_DIRECT);
      ...
      if (err == -1)
         return 0;
      if (inst->dst_da16_subreg_nr())
         format(file, ".%d", inst->dst_da16_subreg_nr() /
                reg_type_size[inst->dst_reg_type()]);
      string(file, "<1>");
      err |= control(file, "writemask", m_writemask, inst->da16_writemask(), NULL);
      err |= control(file, "dest reg encoding", m_reg_type, inst->dst_reg_type(), NULL);
   }
...
dest_3src(FILE *file, gen8_instruction *inst)
{
   ...
   err |= reg(file, BRW_GENERAL_REGISTER_FILE, inst->dst_3src_reg_nr());
   if (err == -1)
      return 0;
   if (inst->dst_3src_subreg_nr())
      format(file, ".%d", inst->dst_3src_subreg_nr());
   string(file, "<1>");
   err |= control(file, "writemask", m_writemask, inst->dst_3src_writemask(), NULL);
   err |= control(file, "dest reg encoding", m_reg_type,
                  three_source_type_to_reg_type(inst->dst_3src_type()), NULL);
         ...
         unsigned swz_w)
{
   ...
   err |= control(file, "vert stride", m_vert_stride, vert_stride, NULL);
   string(file, ",4,1>");
   src_swizzle(file, swz_x, swz_y, swz_z, swz_w);
...
{
   int err = 0;
   unsigned swiz = inst->src0_3src_swizzle();
   unsigned swz_x = (swiz >> 0) & 0x3;
   unsigned swz_y = (swiz >> 2) & 0x3;
   unsigned swz_z = (swiz >> 4) & 0x3;
   unsigned swz_w = (swiz >> 6) & 0x3;

   err |= control(file, "negate", m_negate, inst->src0_3src_negate(), NULL);
   err |= control(file, "abs", m_abs, inst->src0_3src_abs(), NULL);

   err |= reg(file, BRW_GENERAL_REGISTER_FILE, inst->src0_3src_reg_nr());
   if (err == -1)
      return 0;
   if (inst->src0_3src_subreg_nr())
      format(file, ".%d", inst->src0_3src_subreg_nr());
   string(file, "<4,1,1>");
   err |= control(file, "src da16 reg type", m_reg_type,
                  three_source_type_to_reg_type(inst->src_3src_type()), NULL);
   src_swizzle(file, swz_x, swz_y, swz_z, swz_w);
   return err;
}
...
{
   int err = 0;
   unsigned swiz = inst->src1_3src_swizzle();
   unsigned swz_x = (swiz >> 0) & 0x3;
   unsigned swz_y = (swiz >> 2) & 0x3;
   unsigned swz_z = (swiz >> 4) & 0x3;
   unsigned swz_w = (swiz >> 6) & 0x3;
   unsigned src1_subreg_nr =
      (inst->src1_3src_subreg_lo() | (inst->src1_3src_subreg_hi() << 2));

   err |= control(file, "negate", m_negate, inst->src1_3src_negate(), NULL);
   err |= control(file, "abs", m_abs, inst->src1_3src_abs(), NULL);

   err |= reg(file, BRW_GENERAL_REGISTER_FILE, inst->src1_3src_reg_nr());
   if (err == -1)
      return 0;
   if (src1_subreg_nr)
...
{
   int err = 0;
   unsigned swiz = inst->src2_3src_swizzle();
   unsigned swz_x = (swiz >> 0) & 0x3;
   unsigned swz_y = (swiz >> 2) & 0x3;
   unsigned swz_z = (swiz >> 4) & 0x3;
   unsigned swz_w = (swiz >> 6) & 0x3;

   err |= control(file, "negate", m_negate, inst->src2_3src_negate(), NULL);
   err |= control(file, "abs", m_abs, inst->src2_3src_abs(), NULL);

   err |= reg(file, BRW_GENERAL_REGISTER_FILE, inst->src2_3src_reg_nr());
   if (err == -1)
      return 0;
   if (inst->src2_3src_subreg_nr())
      format(file, ".%d", inst->src2_3src_subreg_nr());
   string(file, "<4,1,1>");
   err |= control(file, "src da16 reg type", m_reg_type,
                  three_source_type_to_reg_type(inst->src_3src_type()), NULL);
   /*
...
static int
imm(FILE *file, unsigned type, gen8_instruction *inst)
{
   switch (type) {
   case BRW_REGISTER_TYPE_UD:
      format(file, "0x%08xUD", inst->src1_imm_ud());
      break;
   case BRW_REGISTER_TYPE_D:
      format(file, "%dD", (int) inst->src1_imm_d());
      break;
   case BRW_REGISTER_TYPE_UW:
      format(file, "0x%04xUW", (uint16_t) inst->src1_imm_ud());
      break;
   case BRW_REGISTER_TYPE_W:
      format(file, "%dW", (int16_t) inst->src1_imm_d());
      break;
   case BRW_REGISTER_TYPE_UB:
      format(file, "0x%02xUB", (int8_t) inst->src1_imm_ud());
      break;
   case BRW_REGISTER_TYPE_VF:
      format(file, "Vector Float");
      break;
   case BRW_REGISTER_TYPE_V:
      format(file, "0x%08xV", inst->src1_imm_ud());
      break;
   case BRW_REGISTER_TYPE_F:
      format(file, "%-gF", inst->src1_imm_f());
   }
   return 0;
}

static int
src0(FILE *file, gen8_instruction *inst)
{
   if (inst->src0_reg_file() == BRW_IMMEDIATE_VALUE)
      return imm(file, inst->src0_reg_type(), inst);

   if (inst->access_mode() == BRW_ALIGN_1)
   {
      assert(inst->src0_address_mode() == BRW_ADDRESS_DIRECT);
      return src_da1(file,
                     inst->src0_reg_type(),
                     inst->src0_reg_file(),
                     inst->src0_vert_stride(),
                     inst->src0_da1_width(),
                     inst->src0_da1_hstride(),
                     inst->src0_da_reg_nr(),
                     inst->src0_da1_subreg_nr(),
                     inst->src0_abs(),
                     inst->src0_negate());
   }
   else
   {
      ...
      return src_da16(file,
                      ...
                      inst->src0_vert_stride(),
                      inst->src0_da_reg_nr(),
                      inst->src0_da16_subreg_nr(),
                      inst->src0_abs(),
                      inst->src0_negate(),
                      inst->src0_da16_swiz_x(),
                      inst->src0_da16_swiz_y(),
                      inst->src0_da16_swiz_z(),
                      inst->src0_da16_swiz_w());
   }
}

static int
src1(FILE *file, gen8_instruction *inst)
{
   if (inst->src1_reg_file() == BRW_IMMEDIATE_VALUE)
      return imm(file, inst->src1_reg_type(), inst);

   if (inst->access_mode() == BRW_ALIGN_1)
   {
      assert(inst->src1_address_mode() == BRW_ADDRESS_DIRECT);
      return src_da1(file,
                     inst->src1_reg_type(),
                     inst->src1_reg_file(),
                     inst->src1_vert_stride(),
                     inst->src1_da1_width(),
                     inst->src1_da1_hstride(),
                     inst->src1_da_reg_nr(),
                     inst->src1_da1_subreg_nr(),
                     inst->src1_abs(),
                     inst->src1_negate());
   }
   else
   {
      ...
      return src_da16(file,
                      ...
                      inst->src1_vert_stride(),
                      inst->src1_da_reg_nr(),
                      inst->src1_da16_subreg_nr(),
                      inst->src1_abs(),
                      inst->src1_negate(),
                      inst->src1_da16_swiz_x(),
                      inst->src1_da16_swiz_y(),
                      inst->src1_da16_swiz_z(),
                      inst->src1_da16_swiz_w());
   }
}
...
static int
qtr_ctrl(FILE *file, gen8_instruction *inst)
{
   int qtr_ctl = inst->qtr_control();
   int exec_size = esize[inst->exec_size()];
...
int
gen8_instruction::disassemble(FILE *file, int gen)
{
   int err = 0;
   int space = 0;

   const int opcode = this->opcode();

   if (this->pred_control()) {
      string(file, "(");
      err |= control(file, "predicate inverse", m_pred_inv, this->pred_inv(), NULL);
      format(file, "f%d", this->flag_reg_nr());
      if (this->flag_subreg_nr())
         format(file, ".%d", this->flag_subreg_nr());
      if (this->access_mode() == BRW_ALIGN_1) {
         err |= control(file, "predicate control align1", m_pred_ctrl_align1,
                        this->pred_control(), NULL);
      } else {
   ...
   err |= control(file, "saturate", m_saturate, this->saturate(), NULL);
   err |= control(file, "debug control", m_debug_ctrl, this->debug_control(), NULL);

   if (opcode == BRW_OPCODE_MATH) {
      string(file, " ");
      err |= control(file, "function", m_math_function, this->math_function(),
                     NULL);
   } else if (opcode != BRW_OPCODE_SEND && opcode != BRW_OPCODE_SENDC) {
      err |= control(file, "conditional modifier", m_conditional_modifier,
                     this->cond_modifier(), NULL);

      /* If we're using the conditional modifier, print the flag reg used. */
      if (this->cond_modifier() && opcode != BRW_OPCODE_SEL) {
         format(file, ".f%d", this->flag_reg_nr());
         if (this->flag_subreg_nr())
            format(file, ".%d", this->flag_subreg_nr());
      }
   ...
   if (opcode != BRW_OPCODE_NOP) {
      string(file, "(");
      err |= control(file, "execution size", m_exec_size, this->exec_size(), NULL);
      string(file, ")");
   }
   ...
   if (m_opcode[opcode].nsrc == 3) {
      ...
      if (m_opcode[opcode].ndst > 0) {
         ...
         format(file, " %d %d", this->jip(), this->uip());
      }
   ...
   if (opcode == BRW_OPCODE_SEND || opcode == BRW_OPCODE_SENDC) {
      ...
      case BRW_SFID_SAMPLER:
         format(file, " (%d, %d, %d, %d)",
                this->binding_table_index(),
                this->sampler(),
                this->sampler_msg_type(),
                this->sampler_simd_mode());
         break;

      case BRW_SFID_URB:
         space = 1;
         err |= control(file, "urb opcode", m_urb_opcode,
                        this->urb_opcode(), &space);
         err |= control(file, "urb interleave", m_urb_interleave,
                        this->urb_interleave(), &space);
         format(file, " %d %d",
                this->urb_global_offset(), this->urb_per_slot_offset());
         break;
      ...
      if (space)
         string(file, " ");
      format(file, "mlen %d", this->mlen());
      format(file, " rlen %d", this->rlen());
   }
   pad(file, 64);
   if (opcode != BRW_OPCODE_NOP) {
      string(file, "{");
      space = 1;
      err |= control(file, "access mode", m_access_mode, this->access_mode(), &space);
      err |= control(file, "mask control", m_maskctrl, this->mask_control(), &space);
      err |= control(file, "DDClr", m_dep_clear, this->no_dd_clear(), &space);
      err |= control(file, "DDChk", m_dep_check, this->no_dd_check(), &space);

      err |= qtr_ctrl(file, this);

      err |= control(file, "thread control", m_thread_ctrl, this->thread_control(), &space);
      err |= control(file, "acc write control", m_accwr, this->acc_wr_control(), &space);
      if (opcode == BRW_OPCODE_SEND || opcode == BRW_OPCODE_SENDC)
         err |= control(file, "end of thread", m_eot, this->eot(), &space);
      if (space)
         string(file, " ");
      string(file, "}");
   }
